...
def test_wrong_compression(monkeypatch):
    ...
    assert "Compression lzh for private key is not supported." == e.value.message

def test_none_compression(monkeypatch):
    extra_environment_variables = {
        'bless_ca_default_password': '<INSERT_DEFAULT_KMS_ENCRYPTED_BASE64_ENCODED_PEM_PASSWORD_HERE>',
...
def test_zlib_positive_compression(monkeypatch):
    extra_environment_variables = {
        ...
        'bless_ca_ca_private_key': base64.b64encode(zlib.compress('<INSERT_YOUR_ENCRYPTED_PEM_FILE_CONTENT>')),
    }
...
def test_zlib_negative_compression(monkeypatch):
    extra_environment_variables = {
        'bless_ca_default_password': '<INSERT_DEFAULT_KMS_ENCRYPTED_BASE64_ENCODED_PEM_PASSWORD_HERE>',
        'bless_ca_ca_private_key_compression': 'zlib',
        'bless_ca_ca_private_key': base64.b64encode('<INSERT_YOUR_ENCRYPTED_PEM_FILE_CONTENT>'),
